...
class TelegramBot:
    def __init__(self, openai_client: OpenAIClient, vision_client: VisionClient, config: dict):
    ...
    async def add_user_to_db(self, user):
        if not self.repository.user_exists(user.id):
            self.repository.insert_user(user.id, user.username, user.first_name, user.last_name)
    ...
    async def stats_command(self, update: Update, context: CallbackContext) -> None:
        ...
        result_string = "Your stats so far:\n"
        for service, count in result.items():
            result_string += f"{service}: {count}\n"
    ...
    async def image_command(self, update: Update, context: CallbackContext) -> None:
        ...
        if validate_user_input(user_input):
            ...
            self.repository.update_request_count(update.effective_user.id, "image_generation")
        else:
    ...
    async def tts_command(self, update: Update, context: CallbackContext) -> None:
        ...
        if validate_user_input(user_input):
            ...
            self.repository.update_request_count(update.effective_user.id, "text-to-speech")
            if response.exists():
                response.unlink()
        else:
            await update.message.reply_text("Please provide valid input.")
