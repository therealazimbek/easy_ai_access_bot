...
class UserRepository:
    ...
    def update_request_count(self, user_id, service_name):
        request = self.session.query(Request).filter_by(user_id=user_id, service_name=service_name).first()
        if request:
    ...
    def get_service_counts(self, user_id):
        result = self.session.query(Request.service_name, Request.request_count).filter_by(user_id=user_id).all()
